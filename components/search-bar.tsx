...
const SearchBar = ({ onSearch, icon, searching, setSearching }: { onSearch: (query: string) => void, icon: JSX.Element, searching: boolean, setSearching: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <Button size="icon" className="w-10 h-10 disabled:bg-muted-foreground disabled:opacity-100 " onClick={() => { setSearching(true); onSearch(searchInput.trim()) }} disabled={searching || !(searchInput.trim().length > 0)}>
                <ArrowRightIcon />
            </Button>
        </div>
    )
